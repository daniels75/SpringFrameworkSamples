...
var listIcons = {
    ...
    wanrep : "icon-eject",
    client: "icon-th",
    clientDetail: "icon-th"
}

function refreshpage(repeatSeconds) {
    default_op = "memberlist_instancelist_alertPopup_detectVersionMismatch_getLicenseInfo_getClusterVersion_anyNotifications_getNumberOfClients_lastLoginTime"
    ...
    if (activeView == 'map') {
        ...
        data = prepareWanTopRepDataRequest();
    }
    else if (activeView == 'hotrestart') {
        sendGetClusterHotRestartStatusOperation();
        getMembersHotRestartStatus();
        sendGetClusterHRBackupStatusOperation();
    }
    else if (activeView == 'clusterstate') {
        data = prepareGetClusterStateRequest();
    }
    else if (activeView == 'client') {
        data = prepareClientDataRequest();
    }
    else if (activeView == 'clientDetail') {
        data = prepareClientDetailRequest();
    }
...
function instanceList(container, data, instanceType) {
    ...
    if (data === null || data.length === 0) {
        containerSel.append("<div style='height: 1px'></div>");
        return;
    }
    for (var i = 0; i < data.length; i++) {
        var li = $("<li>");
        var a = $("<a>", {
            href: "#",
            title: data[i].label,
            onclick: "tabs.newTab(\"" + data[i].label + "\", \"" + instanceType + "\", \"" + listIcons[instanceType] + "\")"
        });
        var icon = $("<i>", {
...
function instancelist(data) {
    ...
    multimaplist(data.multimaplist);
    topiclist(data.topiclist);
    reliabletopiclist(data.reliabletopiclist);
    executorlist(data.executorlist);
    wanlist(data.wanlist);
    cachelist(data.cachelist);
    replicatedmaplist(data.replicatedmaplist);
}

function lastLoginTime(data) {
    var lastLoginTimeLabelSel = $("#lastLoginTimeLabel")
    if (data !== 0) {
        lastLoginTimeLabelSel.text(new Date(data).toLocaleString());
    } else {
        lastLoginTimeLabelSel.text("N/A");
    }
}
...
function prepareWanTopRepDataRequest() {
    return {
        cluster: activeCluster,
        operation: default_op + "_getWanSyncState_mapListForWanSync_wanConfigList",
        curtime: 0
    }
}

function prepareClientDataRequest() {
    return {
        cluster: activeCluster,
        operation: default_op + "_clientData",
        type: activeView
    }
}

function prepareClientDetailRequest() {
    return {
        cluster: activeCluster,
        operation: default_op + "_clientDetails",
        type: activeView,
        uuid: activeObject
    }
}
