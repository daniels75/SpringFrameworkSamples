...
var chartOptions = {
    ...
    clientDetail: {
        clientMemoryChart: "clientMemoryChart"
    }
...
var chartTypes = {
    ...
    "clientMemoryChart": "Heap Memory Utilization"
};
...
function charts(data) {
    switch (data.instanceType) {
        case "map":
            drawChart("mapChart1", "map", data.data1);
            drawChart("mapChart2", "map", data.data2);
            drawChart("mapChart3", "map", data.data3);
            drawChart("mapChart4", "map", data.data4);
            break;
        case "cache":
            drawChart("cacheChart1", "cache", data.data1);
            drawChart("cacheChart2", "cache", data.data2);
            drawChart("cacheChart3", "cache", data.data3);
            drawChart("cacheChart4", "cache", data.data4);
            break;
        case "replicatedmap":
            drawChart("replicatedmapChart1", "replicatedmap", data.data1);
            drawChart("replicatedmapChart1", "replicatedmap", data.data2);
            drawChart("replicatedmapChart1", "replicatedmap", data.data3);
            drawChart("replicatedmapChart1", "replicatedmap", data.data4);
            break;
        case "queue":
            drawChart("queueChart1", "queue", data.data1);
            drawChart("queueChart2", "queue", data.data2);
            drawChart("queueChart3", "queue", data.data3);
            break;
        case "topic":
            drawChart("topicChart1", "topic", data.data1);
            drawChart("topicChart2", "topic", data.data2);
            break;
        case "reliabletopic":
            drawChart("reliableTopicChart1", "reliabletopic", data.data1);
            drawChart("reliableTopicChart2", "reliabletopic", data.data2);
            break;
        case "multimap":
            drawChart("multimapChart1", "multimap", data.data1);
            drawChart("multimapChart2", "multimap", data.data2);
            drawChart("multimapChart3", "multimap", data.data3);
            drawChart("multimapChart4", "multimap", data.data4);
            break;
        case "executor":
            drawChart("executorChart1", "executor", data.data1);
            drawChart("executorChart2", "executor", data.data2);
            drawChart("executorChart3", "executor", data.data3);
            drawChart("executorChart4", "executor", data.data4);
            break;
    }
}
...
function requestData() {
    var data = {
        ...
        operation: "getChartDataForPopup",
...
function getChartDataForPopup(datalist) {
    dataCache = datalist;
    if ($.urlParam('chartType') == "memberMemoryChart" || $.urlParam('chartType') == "clientMemoryChart")
...
function drawChart(container, type, datalist, multiple) {
    ...
    if (multiple != null && multiple == true) {
        if (chartOptions[type][container] == "memberMemoryChart") {
            ...
            chartData[2]["label"] = "Total"
        } else if (chartOptions[type][container] == "clientMemoryChart") {
            chartData[0]["data"] = datalist["heap"];
            chartData[0]["label"] = "Max"
            chartData[1]["data"] = datalist["heapMemory"];
            chartData[1]["label"] = "Used"
