...
function getAlertFilterNames(olist){
    if(olist == "")
...
function saveFilterCheckName() {
    ...
    if (filterName === "") {
        ...
        $("#saveFilterNoNameDialog").dialog("option", "buttons", [{
            id: "saveFilterNoNameCancel", text: "OK",
            click: function () {
                $(this).dialog("close");
            }
        }]);
        $("#saveFilterNoNameDialog").dialog("open");
    } else if (!alphanumericWithDashAndUnderscore.test(filterName)) {
        $.growl.error({message: "Filter name may only contain alpha-numeric characters, underscores and dashes."});
    } else {
...
function saveFilterCheckExist() {
    ...
    if ($("#filterListDiv").find("> tbody > #filterName_" + filterName).length) {
        var saveFilterDialog = $("#saveFilterDialog");
        saveFilterDialog.dialog("option", "width", 500);
        saveFilterDialog.dialog("option", "height", 210);
        saveFilterDialog.dialog("option", "buttons", [
            {
                id: "saveFilterOverwrite", text: "Overwrite",
                click: function () {
            ...
            {
                id: "saveFilterCancel", text: "Cancel",
                click: function () {
                    $(this).dialog("close")
                }
            }
...
function saveAlertFilter(olist){
    if (olist === "success") {
        ...
        $.growl.error({message: olist});
    }
}

function saveMemberAlertFilter(data) {
    saveAlertFilter(data);
}

function saveDataTypeAlertFilter(data) {
    saveAlertFilter(data);
}
...
function getAlertFilter(data) {
    if (!data.alertFilterType) {
        $.growl.warning({message: data});
    }
    if (data.alertFilterType === "memberAlertFilter") {
        loadMemberAlertFilter(data);
    } else if (data.alertFilterType === "dataTypeAlertFilter") {
        loadDataTypeAlertFilter(data);
    }
}
...
function fillDataTypeAction(dataType) {
    ...
    var nameFilterListSel = $('#nameFilterList');
    nameFilterListSel.empty();
    ...
    var allRow = $("<tr>");
    var allCheckboxColumn = $("<td>");
    var allLabel = $("<label>", {"class": "checkbox"});
    allLabel.append($("<input>", {
        type: "checkbox",
        name: "nameFilterList",
        id: "checkbox_nameFilter_all",
        click: function () {
            $("input[name=nameFilterList]").attr("checked", $(this).is(":checked"));
        }
    }));
    allCheckboxColumn.append(allLabel);
    allRow.append(allCheckboxColumn);
    allRow.append($("<td>", {text: "All"}));
    tbody.append(allRow);
    ...
    for (var i = 0; i < olist.length; i++) {
        ...
        label.append($("<input>", {
            type: "checkbox",
            name: "nameFilterList",
            id: "checkbox_nameFilter_" + olist[i].label,
            click: function () {
                if (!$(this).is(":checked")) { 
                    $("#checkbox_nameFilter_all").attr("checked", false); 
                }
            }
        }));
        checkboxColumn.append(label);
        row.append(checkboxColumn);
        row.append($("<td>", {text: olist[i].label}));
        tbody.append(row);
    }
    nameFilterListSel.append(table);
    nameFilterListSel.append($("<br>"));
