...
function sendScript() {
    ...
    var lang = $('#radioscriptlanguage').find('option:selected').val()
...
function loadScriptNames(olist) {
    ...
    if (olist.length > 0) {
        if (scriptNameCache.trim() != "") {
            loadScriptAction(scriptNameCache);
        } else {
            loadScriptAction(olist[0])
        }
    }
}
