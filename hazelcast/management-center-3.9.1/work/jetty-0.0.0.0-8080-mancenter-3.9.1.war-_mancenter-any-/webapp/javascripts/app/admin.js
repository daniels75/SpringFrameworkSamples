...
function updateUserAction() {
    ...
    var data = {operation:"updateuser", username:username, admin:admin, roles:roles}
    opcall(data)
}
...
function showSocketInterceptor() {
    var data = {operation: "getSocketInterceptorConfiguration"}
    opcall(data)
}

function getSocketInterceptorConfiguration(data) {
    var configureSocketInterceptorDialog = $("#configureSocketInterceptorDialog")

    configureSocketInterceptorDialog.dialog('open')
    $("#socketInterceptorClassInput").val(data.socketInterceptorClass)
    $("#socketInterceptorParametersTextArea").val(data.configuration)
    configureSocketInterceptorDialog.keypress(function(e) {
...
function configureSocketInterceptorAction() {
    var socketInterceptor = $("#socketInterceptorClassInput").val()
    var configuration = $("#socketInterceptorParametersTextArea").val()
    var data = {
        operation: "configureSocketInterceptor",
...
function disableSocketInterceptorAction() {
    opcall({operation: "disableSocketInterceptor"})
}

function configureSocketInterceptor(data) {
    if (data == 'success') {
        $.growl.notice({message: 'Configured socket interceptor successfully.'})
        $("#configureSocketInterceptorDialog").dialog('close');
    } else {
        $.growl.error({message: 'Failed to configure socket interceptor: ' + data})
    }
}

function disableSocketInterceptor(data) {
    if (data == 'success') {
        $.growl.notice({message: 'Disabled socket interceptor successfully.'})
        $("#configureSocketInterceptorDialog").dialog('close');
    } else {
        $.growl.error({message: 'Failed to configure disable socket interceptor: ' + data})
    }
}