...
var slowOperationDetailTemplate =
    ...
    '    </tr>';
var alphanumericWithDashAndUnderscore = /^[a-z0-9_\-]+$/i;
var scriptNameCache = "";
...
$(document).ready(function () {
    ...
    var loginError = getURLParameterByName("login_error");
    if (loginError === "0") {
        $.growl.error({message: "Wrong username or password!"});
    } else if (loginError) {
        if (isAllDigits(loginError)) {
            var ms = Number(loginError);
            $.growl.error({message: "Login disabled temporarily. Please wait " + (ms < 1000 ? "less than a second" :
                Math.floor(ms / 1000) + " seconds") + " to try again."});
        }
    }
...
function anyNotifications(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var additionalClass = ''
        if (data[i].type == 'INFO') {
            additionalClass = 'alert-info'
        } else if (data[i].type == 'WARN') {
            additionalClass = 'alert-block'
        } else if (data[i].type == 'ERROR') {
            additionalClass = 'alert-error'
        }
        var div = $('<div/>', {
            'class': 'alert ' + additionalClass,
        ...
        div.append(strong)
        div.append(span)
        div.appendTo('#notificationContainer')
    }
}
...
function switchCluster(clusterName) {
    ...
    $('#activeClusterName').text(clusterName);
    tabs.closeAll();
...
function showChangeVersionScreen() {
    sendGetClusterVersionOperation();
    $("#changeVersionDialog").dialog("open");
}

function getClusterVersion(version) {
    activeClusterVersion = version;
    $("#currentVersionContainer").html(version);
}

function sendGetClusterVersionOperation() {
    var data = {
...
function showSlowOperationDetail(data) {
    var detailHtml = slowOperationDetailHeaderTemplate.replace(/\$stackTrace/g, escapeHtml(data[4]));
    var details = data[3];
    if (details && details.length > 0) {
        for (var i = 0; i < details.length; i++) {
            detailHtml += slowOperationDetailTemplate
                .replace(/\$operationDetails/g, escapeHtml(details[i].operationDetails))
                .replace(/\$startTime/g, escapeHtml(moment(details[i].startedAt).format("dddd, MMMM Do YYYY, h:mm:ss a")))
                .replace(/\$duration/g, escapeHtml(details[i].durationMs));
        }
...
function initDialogs() {
        ...
        buttons: [
            {
                ...
                sendChangeClusterVersionOperation(activeClusterVersion)
            }
            }
        ]
    });
    ...
    $("#configureSocketInterceptorDialog").dialog({
        autoOpen: false,
        height: 400,
        width: 600,
        modal: true,
        buttons: [
            {
                id: "configureSocketInterceptorButton",
                text: "Configure",
                click: function () {
                    configureSocketInterceptorAction()
                }
            },
            {
                id: "disableSocketInterceptorButton",
                text: "Disable",
                click: function () {
                    disableSocketInterceptorAction()
                }
            }
        ]
    });
    ...
    $("#forceStartDialog").dialog({
            ...
            buttons: {
                ...
                "Force Start": function () {
                    ...
                    //hotrestart.js
                    sendForceStartClusterOperation();
                    $(this).dialog("close");
                }
            }
    });
